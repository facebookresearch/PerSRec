...
def compute_flash_attention(
    ...
    Args:
        q, k, v: [bs, seq_len, num_attention_heads, attn_head_size]. Three should have the same seq_len.
            I.e., kv state won't be supported here
        attention_mask: bs x seq_len, where >=0 means valid item and otherwise is masked out.
        cu_input_lens: it is the cumulative sum of the input lengths in the batch. It improves efficiency.
            If None, it will be computed from attention_mask.
    """
    ...
    if cu_input_lens is not None:
        ...
        raise ValueError("unexpected usage here")
        return flash_self_attention(qkv, causal=causal, training=training, attention_dropout=attention_dropout)
    else:
        raise ValueError("unexpected usage here, please provide cu_input_lens")
