...
class SEQTrainDataset(Dataset):
    def __init__(self, config, interaction_data):
        self.interaction_data = interaction_data
        self.config = config

        self.item_num = interaction_data.item_num
        self.train_item_seq = interaction_data.train_feat['item_seq']
        self.user_id = interaction_data.train_feat['user_id']

        self.length = len(self.train_item_seq)
        ...
        self.env = None  # keep a None env to be compatible with TextSEQTrainDataset
...
class TextSEQTrainDataset(Dataset):
    def __init__(self, config, interaction_data):
        ...
        self.user_id = interaction_data.train_feat['user_id']
        self.length = len(self.train_item_seq)
        self.train_time_seq = interaction_data.train_feat['time_seq']
        # this is np.ndarray of str
        self.id2token = interaction_data.id2token['item_id']
        ...
        if self.local_rank == 0:
            logger.info(f"load_content on rank {self.local_rank}")
            env = self.load_content(self.text_path, self.text_keys)
            if len(env) < self.item_num:
                logger.warning(f"Text Item num: {len(env)=} < {self.item_num=}. Will do padding")
                env.extend([{}] * (self.item_num - len(env)))
            elif len(env) > self.item_num:
                logger.warning(f"Text Item num: {len(env)=} > {self.item_num=}.")

        else:
            logger.info(f"skip load_content on rank {self.local_rank}")
            env = []
    ...
    @staticmethod
    def load_content(text_path:str, text_keys:List[str]) -> Dict[str, Dict[str, str]]:
        if not text_path.endswith('.parquet'):
            logger.info("Loading text from parquet")
            raise ValueError(f"Unsupported file format: {text_path}")

        # object_id=product_id (brand_id + c2_id)
        df = pl.read_parquet(text_path).select(text_keys + ['object_id']).rename({"object_id": "item_id"})

        env: List[Dict[str, str]] = [{}] # for id 0
        env.extend(df.to_dicts())
        del df
        logger.info(f"Text Item num: {len(env)}")
        return env
    ...
    def __getitem__(self, index):
        ...
        item_seq_token = item_seq
        neg_items_token = neg_item
        ...
        def process_item(item):
            ...
            if item != "[PAD]":
                item = int(item)
                # starts from 1
                assert 1 <= item < len(self.env), f"item {item} out of range of {1}-{len(self.env)}"
                # could be an empty dictionary {}
                item_i = self.env[item]
            text_str = ""
            if item_i:  # if item is 0 (i.e. [PAD]), it is {}
        ...
        outputs = {
            ...
            "user_id": torch.as_tensor(user_id, dtype=torch.int64),
        }
        return outputs
