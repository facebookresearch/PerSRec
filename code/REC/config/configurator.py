...
import os
from REC.utils import _PATH_MANAGER as PathManager
import re
import yaml
...
def does_file_exist(file_name):
    if file_name.startswith("manifold://"):
        return PathManager.exists(file_name)
    else:
...
def get_file_name(file_name):
    if file_name.startswith("manifold://"):
        try:
            return PathManager.get_local_path(file_name, recursive=True)
        except:
            # ok, not a folder, try file
            return PathManager.get_local_path(file_name, recursive=False)
    else:
        return file_name
...
class Config(object):
    ...
    def _load_config_files(self, file_list):
        ...
        if file_list:
            for file in file_list:
                if file.startswith("manifold://"):
                    file = PathManager.get_local_path(file)
                print(f"Loading config file {file}")
                with open(file, 'r', encoding='utf-8') as f:
                    file_config_dict.update(yaml.load(f.read(), Loader=self.yaml_loader))
        print(f"config loaded from file: {file_config_dict}")
    ...
    def __getattr__(self, item):
        ...
        if item in self.final_config_dict:
            value = self.final_config_dict[item]
            if item != "checkpoint_dir" and isinstance(value, str) and value.startswith("manifold://"):
                return get_file_name(value)
            return value
        raise AttributeError(f"'Config' object has no attribute '{item}'")

    def __getitem__(self, item):
        # hack to handle manifold path except checkpoint_dir
        if item in self.final_config_dict:
            value = self.final_config_dict[item]
            if item != "checkpoint_dir" and isinstance(value, str) and value.startswith("manifold://"):
                return get_file_name(value)
