...
def ensure_dir(dir_path):
    ...
    """
    if dir_path.startswith("manifold://"):
        if not _PATH_MANAGER.exists(dir_path):
            _PATH_MANAGER.mkdirs(dir_path)
...
def get_model(model_name):
    ...
    if "HSTU" in model_name:
        model_file_name = "hstu"
    if "HLLM" in model_name:
        model_file_name = "hllm"
    model_module = None
...
def get_tensorboard(logger, base_path: str = 'log_tensorboard'):
